...
TEST(KeyFetcherGcp, GetPolicy) {
      ...
      .WillOnce(Return(google::cloud::spanner_mocks::MakeRow(
          {{"Policy",
            google::cloud::spanner::Value(google::cloud::spanner::Bytes(
                expected_policy->SerializeAsString()))}})));

  auto mock_connection =
      std::make_shared<google::cloud::spanner_mocks::MockConnection>();

  constexpr absl::string_view kPolicyId = "some_policy";
      ...
      .WillOnce(
          [&mock_result_set_source](
            ...
            google::cloud::StatusOr<google::cloud::spanner::Value> parameter =
                sql_params.statement.GetParameter("policy_id");
            if (!parameter.ok()) return {};
            google::cloud::StatusOr<std::string> policy_id =
                parameter->get<std::string>();
            if (!policy_id.ok()) return {};
            if (*policy_id != kPolicyId) return {};
  ...
  absl::StatusOr<oak::attestation::v1::ReferenceValues> policy =
      policy_fetcher->GetPolicy(kPolicyId);
...
TEST(KeyFetcherGcp, NotFoundError) {
  ...
  std::unique_ptr<PolicyFetcher> policy_fetcher =
      PolicyFetcherGcp::Create(std::move(spanner_client));
  EXPECT_THAT(policy_fetcher->GetPolicy("some_policy"),
              StatusIs(absl::StatusCode::kNotFound,
                       HasSubstr("Cannot find 'some_policy'")));
}
