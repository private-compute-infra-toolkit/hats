...
# It then either outputs to a result file, or console
# If given a result file, it appends if it exists, for a single result file
...
def main():
  ...
  parser.add_argument('--results_json', help='Results json file to store result. If not provided, prints to console instead')
  ...
  subprocess.call(cmd)
  # The command must have produced a summary file
  ...
  status = 'fail'
  exit_code_ok = (('exit_code' not in task_result.keys())
                  or (task_result['exit_code'] == '0'))
  failure = (('failure' in task_result.keys())
             and task_result['failure'])
  internal_failure = (('internal_failure' in task_result.keys())
                      and task_result['internal_failure'])
  if internal_failure:
    ...
    status = 'expired'
        ...
        and exit_code_ok and not failure and not internal_failure):
    status = 'pass'
...
# Add to results json
# This is for a persistent artifact json to be pulled, updated, and uploaded.
# This requires Kokoro cfg updates, and support for checking the artifact.
# Currently, this is just used to save results after manual run.
def write_to_results(args, device, task_result, status):
  ...
  # Look for duplicate results (same timestamp+commit)
  ...
  results[args.build_timestamp]['tests'][device][args.test_name] = {
    ...
    'status': status,
    # Populated by --task-output-stdout=json
