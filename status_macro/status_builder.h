...
#ifndef HATS_STATUS_MACRO_STATUS_BUILDER_H_
#define HATS_STATUS_MACRO_STATUS_BUILDER_H_
...
class [[nodiscard]] StatusBuilder {
 public:
  ...
  explicit StatusBuilder(const absl::Status& original_status,
                         SourceLocation location HATS_LOC_CURRENT_DEFAULT_ARG);
  ...
  // NOTE: Didn't come with common, need multiple extra files to implement
  ...
  //
  //   HATS_RETURN_IF_ERROR(foo()).With(TeamPolicy).With(OtherTeamPolicy);
  //
  // Terminal adaptors are often used for type conversion. This allows
  // HATS_RETURN_IF_ERROR to be used in functions which do not return Status.
  // For example, a function might want to return some default value on error:
  //
  //   int GetSysCounter() {
  //     int value;
  //     HATS_RETURN_IF_ERROR(ReadCounterFile(filename, &value))
  //         .LogInfo()
  ...
  //
  //   bool DoMyThing() {
  //     HATS_RETURN_IF_ERROR(foo()).LogWarning().With(status_macros::Return(false));
  //     ...
  //   }
  //
...
//       .With(util::TaskReturn(task));
//
// Note in the above example, the HATS_RETURN_IF_ERROR macro ensures the
